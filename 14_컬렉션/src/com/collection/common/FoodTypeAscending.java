...
public class FoodTypeAscending implements Comparator{
	...
	public int compare(Object o1, Object o2) {
		...
		if(prev.getType().compareTo(next.getType())==0) { // 타입이 같을 때
			if(prev.getPrice() < next.getPrice()) return +1; // 가격으로 재비교
			else if(prev.getPrice() > next.getPrice()) return -1;
			else return 0;
			//return prev.getPrice()-next.getPrice(); // 오름차순 정렬
			//return next.getPrice()-prev.getPrice(); // 내림차순 정렬
		}
